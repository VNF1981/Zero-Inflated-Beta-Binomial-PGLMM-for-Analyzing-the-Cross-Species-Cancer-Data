...
The beta-binomial component models the underlying cancer probability for each species. The dispersion parameter widens the variance beyond the binomial limit. A custom variance inflation factor (VIF) is calculated from this parameter to summarize how much additional variance is present relative to a binomial model. For example, a VIF = 5 means the observed variance in cancer counts is five times larger than what a standard binomial model could accommodate, which shows why a beta-binomial model is required.
...
#### *This script uses nested loops to fit both simple models (single predictors) and the full multiple predictor model, and it runs each of them independently for the neoplasia and malignancy responses.*
